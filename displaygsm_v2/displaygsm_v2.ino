...
unsigned char text1[] ={"<1s1>Send SMS "};
...
void scrollMessage(int sizexx,int count,char prio) {
    ...
    for(int i=counter+5;i<counter+sizexx;i++){
      ...
      myCharx=EEPROM.read(i-counter);
      if(myChar!=0 || myCharx!=0){
        loadBufferLong(myChar,myCharx,prio);
      }
      else if(myChar==0 && myCharx==0){
        break;
      }
    }
    if (flag==0){
...
void loadBufferLong(int ascii,int ascii2,char prio){
    if (ascii >= 0x20 && ascii <=0x7f && ascii2 >=0x20 && ascii2 <=0x7f){
        for (int a=0;a<7;a++){
        ...
        for (int a=0;a<7;a++){
            unsigned long c = pgm_read_byte_near(font5x7 + ((ascii2 - 0x20) * 8) + a);
            unsigned long x = bufferLong11 [a*2];
            x = x | c;
            bufferLong11 [a*2] = x;
        }
        
        unsigned int count = pgm_read_byte_near(font5x7 +((ascii - 0x20) * 8) + 7);
        unsigned int count2 = pgm_read_byte_near(font5x7 +((ascii2 - 0x20) * 8) + 7);
        int ucount;
        if((count-count2)>=0){
          ucount=count;
        }
